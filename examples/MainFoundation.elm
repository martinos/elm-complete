...
import Complete.Foundation exposing (..)
...
type Msg
    = SelectAccount (C.Msg Movie)
    | SelectCountry (C.Msg String)
...
update msg model =
    case msg of
        SelectAccount op ->
            { model | movie = model.movie |> C.update .name accounts op }

        SelectCountry op ->
            { model | country = model.country |> C.update identity countries op }
